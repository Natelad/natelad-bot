...
SYSTEM_PROMPT = (
    ...
    "*Learn more:* https://nateladagency.com\n"
    "*Contact:* +263 7xx xxx xxx\n"
    "Respond in clean WhatsApp text. Use *asterisks* to bold only key points and headings."
)
...
def generate_response(message):
    if chat:
        try:
            ...
            for word in sorted(keywords_to_bold, key=len, reverse=True):
                pattern = rf'\b{re.escape(word)}\b'
                plain_text = re.sub(pattern, f"*{word}*", plain_text)

            # Strip any unwanted markdown links still lingering
